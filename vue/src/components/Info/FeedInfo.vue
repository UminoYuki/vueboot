...
export default {
  ...
  methods: {
    ...
    flushTime() {
      this.request.get("/echarts/getDate7?type=饲料包装单元").then(res => {
        if (res === null) {
          console.log("时间信息为空")
        } else {
  ...
  mounted() {  // 页面元素渲染好之后才会触发
    ...
    let flushData = () => {
      this.request.get("/echarts/getTemperature7?type=饲料包装单元").then(res => {
        if (res === null) {
          console.log("温度信息为空");
        } else {
      ...
      this.request.get("/echarts/getSquare7?type=饲料包装单元").then(res => {
        if (res === null) {
          console.log("湿度信息为空");
        } else {
          console.log(res.data)
